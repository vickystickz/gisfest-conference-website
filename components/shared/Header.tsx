...
const Header = () => {
  ...
  return (
    <header className={`grid z-30 transition-all delay-150 ease-in-out ${isScrolled ? "fixed laptop:left-0 laptop:right-0 backdrop-blur-blur ": null} ${showMobileMenu && "h-full z-50"} w-full laptop:grid-cols-12 laptop:max-w-screen-tv mx-auto grid-cols-2 laptop:items-center laptop:px-24 px-3 laptop:py-[22px] tablet:px-14 py-4`}>
      ...
      <div className={`fixed laptop:relative transition-all delay-150 ease-in-out top-0 ${showMobileMenu ? "right-0 left-0  bottom-0 z-40": "right-[-120%]"} w-full h-screen  laptop:right-0  laptop:h-max  pt-[30%] tablet:pt-[5.5%] laptop:pt-0 laptop:gap-0 gap-24  tablet:gap-8 tablet:px-14 px-6 laptop:px-0 backdrop-blur-blur flex flex-col laptop:backdrop-blur-none laptop:col-span-10 laptop:grid-cols-12 laptop:grid`}>
      ...
      <div className="col-span-1 flex text-center mt-3 laptop:items-center justify-end laptop:hidden">
        <div onClick={handleMobileMenu} className="transition-all delay-150 ease-in-out z-40">
          {
            showMobileMenu ?
